...
public class MultiShot
{
    ...
    public MultiShot(IGetEmbeddings embeddingClient, ICreateChatCompletions chatClient)
    {
    ...
    public async Task<IEnumerable<ChatContent>> GetChatResponse(string userQuery, IEnumerable<string> documents, IEnumerable<ChatContent>? previousContext)
    {
        ...
        var chatContents = previousContext is null
            ? new List<ChatContent> { ChatContent.From(systemMessage, ChatRole.System) }
            : previousContext.Select(c => c).ToList();

        string queryText = userQuery!;
        if (previousContext is not null)
        {
            // Disambiguate the user's request if this is not the 1st query
            var disambiguationContents = chatContents.GetDisambiguationContents(userQuery);
            var disambiguationResponse = await disambiguationContents.GetChatCompletions(_chatClient);
            queryText = disambiguationResponse.Value ?? queryText;
        }
