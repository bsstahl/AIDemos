...
internal class InputLayer
{
    ...
    public InputLayer(int inputCount, int outputCount, double[] startingWeights, double[] startingBiases, IActivateNeurons? activationFunction = null)
    {
    ...
    public double[] Predict(double[] inputs)
    {
        ...
        return this.ActivationFunction.Activate(inputs.Select((x, i) => this.Weights[i] * x).Sum() + this.Biases);
    }
        ...
        Action<int, Model, double>? callback = null)
    {
        ...
        while (!this.TrainingConverged && this.TrainingIterations < _maxIterations)
        {
            ...
            this.Biases -= _learningRate * deltaB[0];
    ...
    private Model Clone() => new Model(this.Weights.Length, this.Weights, this.Biases)
    {
